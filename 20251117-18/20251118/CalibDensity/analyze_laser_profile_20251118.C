...
void analyze_laser_profile_20251118() {
    ...
    for (int fileNum : file_nums) {
        TString filename = TString::Format("../20251118profile/DeviceData_#%03d.csv", fileNum);
        ...
        projX->Fit(gx, "Q0R"); // Q:Quiet, 0:Do not store, R:Range
    ...
    std::cout << "Peak Position: X=" << center_x << " mm, Y=" << center_y << " mm" << std::endl;
    ...
    // オフセット付きガウスフィット
    TF1 *gx = new TF1("gx", "gaus + pol0(3)", center_x - 3.0, center_x + 3.0); // ピーク周辺±3mmでフィット
    gx->SetLineColor(kRed);
    gx->SetParameters(global_max_val, center_x, 0.5, 200); // 高さ, 中心, sigma, オフセット(適当)
    projX->Fit(gx, "R"); // Range指定でフィット
