void normalize_pinholeon_data() {
    ...
    // Exponential fit: y = A + B * exp(C * t) (20251118, ほぼ一定)
    double A = 1559.12;         // オフセット
    double B = -262.836;        // 振幅（負の値）
    double C = 2.15569e-11;     // 増大定数 [/min]（ほぼゼロ）
    ...
    // Pinhole ONのデータ（偶数番号: 2, 4, 6, 8, 10, 12, 14, 16）
    const int nPoints = 8;
    ...
    fit_gauss->SetParameter(0, 200);   // 振幅の初期値（ピークは約206）
    fit_gauss->SetParameter(1, 0.375); // 中心位置の初期値（0.25と0.5の中間）
    fit_gauss->SetParameter(2, 0.3);   // σの初期値（広めに設定）
    fit_gauss->SetParLimits(0, 50, 500);     // 振幅の範囲を制限
    fit_gauss->SetParLimits(1, -1.0, 2.0);   // 中心位置の範囲を広く
    fit_gauss->SetParLimits(2, 0.1, 1.5);    // σの範囲を広く
